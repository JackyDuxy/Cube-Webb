...
1.) Inspection mode
2.) Timer mode
3.) Stops solve, saves solve, generates new scramble
*/
timerMode = 0;
...
let isSolving = false;
// Used to prevent immediate startInspectionTimer() after stop()
let holdTimer = false;
...
function update() {
    ...
    timerDisplay.textContent = timeTranslation(elapsedTime);
}
...
function addTime(time, isPlus2, isDNF, scramble) {
    ...
    plus2.addEventListener('click', () => {
        ...
        if (!times[index].isDNF) {
            ...
            if (times[index].isPlus2) {
                times[index].time = times[index].time = times[index].time + 2000;;
                plus2.style.color = "rgb(250, 128, 114)";
                button.textContent = (index + 1) + '. +' + timeTranslation(times[index].time);
            } else {
                times[index].time = times[index].time = times[index].time - 2000;;
                plus2.style.color = "rgb(15, 10, 35)";
                button.textContent = (index + 1) + ". " + timeTranslation(times[index].time);
            }
            return button, times;
        }
    ...
    dnf.addEventListener('click', () => {
        ...
        if (times[index].isDNF) {
            button.textContent = (index + 1) + '. DNF';
            dnf.style.color = "rgb(250, 128, 114)";
            return button, times;
        } else {
            button.textContent = (index + 1) + ". " + timeTranslation(times[index].time);
            dnf.style.color = "rgb(15, 10, 35)";
            return button, times;
        }
    });
    // Functionality for removing a solve
    remove.addEventListener('click', () => {
        let index = Number(remove.id);
    ...
    if (times[times.length - 1].isDNF) {
        button.textContent = times.length + ". DNF";
        dnf.style.color = "rgb(250, 128, 114)";
    } else if (times[times.length - 1].isPlus2) {
        times[index].time = times[index].time + 2000;
        plus2.style.color = "rgb(250, 128, 114)";
        button.textContent = times.length + ". +" + timeTranslation(times[index].time);
    } else {
        button.textContent = times.length + ". " + timeTranslation(solve.time);
    }
    ...
    dnf.innerText = "DNF";
    solveButtons.appendChild(remove);
    remove.innerText = "X";
...
// Translates elapsed time into display time
function timeTranslation(elapsedTime) {
    ...
    if(event.key == ' ') {
        if (!isInspecting && !isSolving && timeSinceStop > 0) { // starts inspection
            ...
            if (timeHeld > 0) {
                ...
                stopInspectionTimer();
                isInspecting = false;
                timerDisplay.style.color = 'rgb(15, 10, 35)';
                start();
                isSolving = true;
            }
            timeHeld = 0;
